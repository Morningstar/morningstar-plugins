...
class Formatter:
    ...
    def overall_rating(data: Dict, start_date=None, end_date=None) -> str:
        ...
        latest = _latest(hist)
        if latest:
            # Append Rating Type to the Medalist Rating line already queued above
            rating_type = latest.get("Medalist Rating Type")
            if rating_type:
                # replace the last appended line in-place to include the type
                lines[-1] = lines[-1].rstrip() + f"  {_display_rating_type(rating_type)}"
        ...
        # Prepend disclosure if applicable
    ...
    def historical_ratings(data: Dict, start_date=None, end_date=None) -> str:
        ...
        if not hist:
            ...
            return "{}{}." .format(msg, " for the specified date range" if label else "")
        sorted_hist = sorted(hist, key=lambda r: _get_record_date(r), reverse=True)

        lines = [f"**Historical Ratings** ({len(sorted_hist)} months){label}\n"]
        lines.append("| Date | Medalist Rating | Medalist Rating Type | People | People Type | Process | Process Type | Parent | Parent Type | Price |")
        lines.append("|------|-----------------|----------------------|--------|-------------|---------|--------------|--------|-------------|-------|")
    ...
    def price_score(data: Dict, start_date=None, end_date=None) -> str:
        ...
        # Include price text/methodology if present
        if price_text:
        ...
        if algo:
            ...
            if latest_algo:
                ...
                for r in latest_algo:
                    ...
                    lines.append(f"  • {feat}: raw={raw}, ranked={ranked}{w_str}{adj_str}")
        ...
        if algo:
            ...
            if latest_algo:
                ...
                for r in latest_algo:
                    ...
                    rule_str = f"  rule-score={rule}" if rule != "" else ""
                    lines.append(f"  • {feat}: raw={raw}, ranked={ranked}{rule_str}")
        lines.append(f"\n{PILLAR_SCORE_SCALE_LINE}")
    ...
    def _section_overall_breakdown(data: Dict, start_date=None, end_date=None) -> str:
        ...
        all_hist = data.get("historical_ratings", [])
        hist     = _filter_by_date(all_hist, s, e)
        latest   = _latest(hist)
        if latest:
            # Append Rating Type to the Medalist Rating line already queued above
            rating_type = latest.get("Medalist Rating Type")
        ...
        if formula:
            clean = formula.replace("$$", "\n").replace("\\times", "×")
            clean = clean.replace("\\Big(", "(").replace("\\Big)", ")")
            clean = clean.replace("\\text{", "").replace("}", "")
            clean = clean.replace("\\quad", "  ").replace("\\le", "≤").replace("\\ge", "≥")
            lines.append(f"\n**Formula:**\n{clean.strip()}")
        ...
        if not hist:
            ...
            sorted_hist = sorted(hist, key=lambda r: _get_record_date(r), reverse=True)
            lines.append(f"({len(sorted_hist)} months)\n")
        ...
        price_data     = _filter_by_date(price_obj.get("data", []), s, e)
        price_text     = price_obj.get("text", "")
        medalist_price = data.get("medalist_price_score", "N/A")
        lines.append(f"• **Medalist Price Score:** {medalist_price}")
        latest_price = _latest(price_data) if price_data else None
        if latest_price:
            lines.append(f"• **Annual Fee:** {latest_price.get('AnnualFee', 'N/A')}")
            lines.append(f"• **Category Median Annual Fee:** {latest_price.get('CategoryMedianAnnualFee', 'N/A')}")
            lines.append(f"• **Fee Type:** {latest_price.get('FeeType', 'N/A')}")
            raw = latest_price.get("PriceScoreRaw")
            if raw is not None:
                lines.append(f"• **Price Score Raw:** {raw:.4f}")
    ...
    def _section_pillar_analysis(data: Dict, start_date=None, end_date=None) -> str:
        ...
        lines.append("\n" + Formatter.people_pillar(data, start_date=start_date, end_date=end_date))
        lines.append("\n" + Formatter.process_pillar(data, start_date=start_date, end_date=end_date))
        lines.append("\n" + Formatter.parent_pillar(data, start_date=start_date, end_date=end_date))
